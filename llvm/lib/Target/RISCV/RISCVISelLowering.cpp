...
unsigned RISCVTargetLowering::getVectorTypeBreakdownForCallingConv(
  ...
  unsigned NumRegs = TargetLowering::getVectorTypeBreakdownForCallingConv(Context, CC, VT, IntermediateVT, NumIntermediates, RegisterVT);
...
SDValue RISCVTargetLowering::LowerIS_FPCLASS(SDValue Op,
  ...
  SDValue FPCLASS = DAG.getNode(RISCVISD::FPCLASS, DL, XLenVT, Op.getOperand(0));
...
SDValue RISCVTargetLowering::lowerEXTRACT_VECTOR_ELT(SDValue Op,
  ...
  if (VecVT.getVectorElementType() == MVT::i1) {
    ...
    if (isNullConstant(Idx)) {
      ...
      SDValue Res = DAG.getSetCC(DL, XLenVT, Vfirst, DAG.getConstant(0, DL, XLenVT),
                           ISD::SETEQ);
...
static SDValue lowerGetVectorLength(SDNode *N, SelectionDAG &DAG,
  ...
  SDValue Res = DAG.getNode(ISD::INTRINSIC_WO_CHAIN, DL, XLenVT, ID, AVL, Sew, LMul);
...
SDValue RISCVTargetLowering::LowerINTRINSIC_WO_CHAIN(SDValue Op,
  ...
  case Intrinsic::riscv_sm4ed: {
    ...
    if (RV64LegalI32 && Subtarget.is64Bit() && Op.getValueType() == MVT::i32) {
      ...
      SDValue Res = DAG.getNode(Opc, DL, MVT::i64, NewOp0, NewOp1, Op.getOperand(3));
      return DAG.getNode(ISD::TRUNCATE, DL, MVT::i32, Res);
    }
  ...
  case Intrinsic::riscv_vmv_x_s: {
    SDValue Res = DAG.getNode(RISCVISD::VMV_X_S, DL, XLenVT,
                       Op.getOperand(1));
